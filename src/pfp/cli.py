...
def load_portfolio(movements_file, investments_file=None):
    importer = TradeRepublicImporter()
    movements = importer.load(movements_file)

    investments = None
    if investments_file is not None:
        repository = InvestmentRepository(investments_file)
...
def run_recommend(amount, movements_file, investments_file=DEFAULT_INVESTMENTS_FILE):
...
def run_invest(
    ...
    if position is not None:
        print()
        print(
            f"Posición total : {position.shares}"
            f" participaciones"
        )
