...
class WebApp:
    ...
    def _layout(self, content: str, path: str) -> str:
        return f'''<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PFP</title><style>{CSS}</style></head><body><header><div><strong>PFP</strong><span>Personal Finance Portfolio</span></div><div class="header-actions">{navigation_html(path)}</div></header><main>{content}</main><footer class="app-footer"><a href="/help">Ayuda</a></footer></body></html>'''
