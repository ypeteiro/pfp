...
class WebApp:
    ...
    def _movements(self) -> str:
        rows = "".join(f'<tr><td>{escape(str(m.datetime))}</td><td>{escape(m.type or "—")}</td><td>{escape(m.symbol or "—")}</td><td>{euro(m.amount)}</td></tr>' for m in self.report.movements)
