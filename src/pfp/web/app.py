...
class WebApp:
    ...
    def _layout(self, content: str, path: str) -> str:
        return f'''<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PFP</title><style>{CSS}</style></head><body><header><div><strong>PFP</strong><span>Personal Finance Portfolio</span></div>{navigation_html(path)}</header><main>{content}</main><footer class="app-footer"><a href="https://github.com/ypeteiro/pfp#readme" target="_blank" rel="noopener noreferrer">Ayuda y documentación (README)</a></footer></body></html>'''
