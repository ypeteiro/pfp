...
def _evolution_summary(evolution: PatrimonyEvolution, points: tuple[PatrimonyPoint, ...]) -> str:
    ...
    for index in range(5):
        ...
        y = y_for(value)
        ticks.append(f'<line x1="{left}" y1="{y:.1f}" x2="{width-right}" y2="{y:.1f}" style="stroke:#e5e7eb;stroke-width:1" /><text x="{left-12}" y="{y+4:.1f}" text-anchor="end" style="fill:#64748b;font-size:12px">{escape(euro(value))}</text>')
    ...
    for index in label_indexes:
        ...
        date_labels.append(f'<line x1="{x:.1f}" y1="{height-bottom}" x2="{x:.1f}" y2="{height-bottom+7}" style="stroke:#94a3b8;stroke-width:1" /><text x="{x:.1f}" y="{height-20}" text-anchor="middle" style="fill:#475569;font-size:12px">{escape(date)}</text>')

    lines = [f'<polyline points="{polyline(values)}" fill="none" style="stroke:{stroke};stroke-width:{stroke_width};stroke-linecap:round;stroke-linejoin:round;{extra_style}" />' for _, _, values, stroke, stroke_width, extra_style in series]
    points_svg = []
    for index, point in enumerate(points):
        title = escape(f'{point.datetime.strftime("%d/%m/%Y")}: Patrimonio {euro(point.patrimony)} · Aportado {euro(point.cumulative_contributed)} · Capital invertido {euro(point.invested_cost)}')
        for _, _, value, stroke, _, _ in (("patrimony", "Patrimonio", point.patrimony, "#2563eb", 4, ""), ("contributed", "Capital aportado", point.cumulative_contributed, "#64748b", 3, ""), ("invested", "Capital invertido", point.invested_cost, "#059669", 3, "")):
