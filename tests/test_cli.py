...
def test_run_invest_persists_investment(tmp_path):
    ...
    run_invest(symbol="TEST", shares=Decimal("2"), amount=Decimal("200"), portfolio_class="EQUITY", movements_file=MOVEMENTS_FILE, investments_file=investments_file)
...
def test_run_invest_uses_utc_datetime(tmp_path):
    investments_file = tmp_path / "investments.csv"
    run_invest(symbol="TEST", shares=Decimal("1"), amount=Decimal("100"), portfolio_class="EQUITY", movements_file=MOVEMENTS_FILE, investments_file=investments_file)
    investments = InvestmentRepository(investments_file).load()
    assert len(investments) == 1
    assert investments[0].datetime.tzinfo is not None
    assert investments[0].datetime.utcoffset() is not None
...
def test_run_invest_order_uses_current_price_and_persists(tmp_path):
    ...
    run_invest_order(symbol="IE00BG47KH54", amount=Decimal("300"), movements_file=MOVEMENTS_FILE, investments_file=investments_file, price_provider=StubPriceProvider(), sales_file=sales_file)
    investments = InvestmentRepository(investments_file).load()
    assert len(investments) == 1
    investment = investments[0]
...
def test_run_invest_order_rejects_unknown_symbol(tmp_path):
    ...
    try:
        run_invest_order(symbol="UNKNOWN", amount=Decimal("100"), movements_file=MOVEMENTS_FILE, investments_file=investments_file)
...
def test_recommendation_order_can_be_executed_as_investment(tmp_path):
    ...
    recommendation = RecommendationEngine().recommend(portfolio, Decimal("300"))
    order = recommendation.orders[0]
    class StubPriceProvider:
        def get_prices(self, symbols):
            return {order.symbol: Decimal("120")}
    run_invest_order(symbol=order.symbol, amount=order.amount, movements_file=MOVEMENTS_FILE, investments_file=investments_file, price_provider=StubPriceProvider(), sales_file=sales_file)
    investments = InvestmentRepository(investments_file).load()
    assert len(investments) == 1
...
def test_run_recommend_prints_executable_invest_order(tmp_path, capsys):
    ...
    portfolio = load_portfolio(MOVEMENTS_FILE, investments_file, sales_file)
    recommendation = RecommendationEngine().recommend(portfolio, Decimal("300"))
    run_recommend(Decimal("300"), MOVEMENTS_FILE, investments_file, sales_file)
    output = capsys.readouterr().out
    for order in recommendation.orders:
        assert ("python -m pfp invest-order " f"{order.symbol} " f"{order.amount:.2f} " f"{MOVEMENTS_FILE} " f"--investments-file {investments_file}") in output
...
def test_run_sell_persists_sale_and_updates_portfolio(tmp_path):
    ...
    run_sell(symbol="IE00B4L5Y983", shares=Decimal("0.1"), amount=Decimal("150"), movements_file=MOVEMENTS_FILE, investments_file=investments_file, sales_file=sales_file)
